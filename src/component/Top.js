...
const Top = () => {
  document.title='Moviepp | Top_Rated'
  const navigate = useNavigate()
  const [loading, setLoading] = useState(false)
  const [data, setData] = useState([])
  const getdata = async () => {
    setLoading(true)
    const data = await fetch('https://api.themoviedb.org/3/movie/top_rated?api_key=ea98c51f7fbeb1c31b0d29eb704698fc')
  ...
  if(loading){
    ...
    return (
      ...
      <div className='popularcon'>
        <h1 className='results'>Results for "Top Rated"</h1>
